...
/**
 * VIEW
 * refereeing list : club planning view
 *
 * @version $Id: referee_club_planning.php 85 2014-08-27 10:18:51Z lezardro $
...
if( empty($matches)):
	...
	echo $pagination;
?>
	<form name="form_planning" action="<?php echo site_url('arbitrage-club/enregistrer'); ?>" method="post" role="form" class="form-inline">
	...
	<div class="table-responsive">
		...
		<tr>
			<th>Date<span class="inline"><?php
				echo anchor(site_url('arbitrage-club/tri/date-asc'), _img_order_asc_props(), array('class' => 'up-down'));?><?php
				echo anchor(site_url('arbitrage-club/tri/date-desc'), _img_order_desc_props(), array('class' => 'up-down'));?></span></th>
			<th>Heure</th>
			<th>Lieu</th>
			<th>Catégorie<span class="inline"><?php
				echo anchor(site_url('arbitrage-club/tri/cat-asc'), _img_order_asc_props(), array('class' => 'up-down'));?><?php
				echo anchor(site_url('arbitrage-club/tri/cat-desc'), _img_order_desc_props(), array('class' => 'up-down'));?></span></th>
			<th>Equipe 1<span class="inline"><?php
				echo anchor(site_url('arbitrage-club/tri/equ1-asc'), _img_order_asc_props(), array('class' => 'up-down'));?><?php
				echo anchor(site_url('arbitrage-club/tri/equ1-desc'), _img_order_desc_props(), array('class' => 'up-down'));?></span></th>
			<th>Equipe 2<span class="inline"><?php
				echo anchor(site_url('arbitrage-club/tri/equ2-asc'), _img_order_asc_props(), array('class' => 'up-down'));?><?php
				echo anchor(site_url('arbitrage-club/tri/equ2-desc'), _img_order_desc_props(), array('class' => 'up-down'));?></span></th>
			<th>Arbitre 1</th>
...
<?php foreach ($matches as $match): ?>
		<tr>
			...
			if( ! empty($match->referee1_name)):
				...
				$display = $match->referee1_name . ( ! empty($match->referee1_club_name)? ' (' . $match->referee1_club_name . ')':"");
			elseif( ! empty($match->match_referee1_club)):
				$default_referee = 'c;' . $match->match_referee1_club . ';0';
				$display = $match->referee1_club_name;
			else:
				$default_referee = '';
				$display = '';
			endif;
			if($match->match_referee1_club == $current_club):
			...
			echo form_dropdown('referee2[' . $match->match_id . ']', $all_referees, $default_referee, 'class="form-control input-sm"');
			...
			?></td>
			<td><?php
			if($match->match_referee2_club == $current_club):
				echo form_dropdown('referee2_status[' . $match->match_id . ']', $options_action, $match->match_referee2_status, 'class="form-control input-sm"');
			else:
				if (isset($options_action[$match->match_referee2_status])):
					echo $options_action[$match->match_referee2_status];
					echo form_hidden('referee2_status[' . $match->match_id . ']', $match->match_referee2_status);
				else:
					echo form_hidden('referee2_status[' . $match->match_id . ']', 'C');
				endif;
			endif;
			?></td>
		</tr>
