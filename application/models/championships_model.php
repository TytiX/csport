 ...
 *
 * @version $Id: championships_model.php 90 2014-09-22 19:07:05Z lezardro $
...
class Championships_model extends CI_Model
{
	...
	function getAll($criteria, $limit = 0, $nb = CRH_NB_RECORD)
	{
		...
		if(isset($criteria['cactive']) && $criteria['cactive'] != 'all') {
			$this->db->where('cs_active', $criteria['cactive']);
		}
		//-----------------------------
		// ordering
		$this->db->order_by('cs_order asc');
		//--
		$query = $this->db->get();
	...
	/**
	 * return active championships as array
	 * (used for dropdowns)
	 *
	 * @param bool $default, true if there must be a default value
	 *
	 * @return array $cs_array (cs_id => cs_name)
	 */
	function getCsAsArray($default = false)
	{
		$cs_array = array();
		if($default) {
			$cs_array['0'] = '--';
		...
		foreach ($results['result'] as $cs) {
			$cs_array[$cs->cs_id] = $cs->cs_name;
		}
		return $cs_array;
	}
...
/* End of file championships_model.php */
/* Location: ./application/models/championships_model.php */