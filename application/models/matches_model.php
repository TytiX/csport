...
class Matches_model extends CI_Model
{
	 ...
	 */
	function getAllMAtches($criteria, $limit = 0, $nb = CRH_NB_RECORD)
	{
		$result = array();
	 ...
	 */
	function getIncompleteMatches($delay)
	{
	 ...
	 * @return query results
	 */
	function getMatchesNotConfirmed($delay)
	{
		$limit_date_min = date('Y-m-d H:i:s', mktime('00', '00', '00', date('n'), date('j') + $delay, date('Y')));
		$limit_date_max = date('Y-m-d H:i:s', mktime('23', '59', '59', date('n'), date('j') + $delay, date('Y')));
		$this->db->select("match_id, match_datetime,
	 ...
	 */
	function importMatches($fieldname, $new_name)
	{
	 ...
	 * @return int $match_id
	 */
	function setMatch($match_id, $match)
	{
		if(empty($match)) {
	 ...
	 * @return int $match_id
	 */
	function _updateMatch($match_id, $data)
	{
